...
// 1. Wrapper pour bloquer l’exécution concurrente
...
// 2. Fallback CryptoPanic : élargir le filtre si pas d’article « hot »
...
// 3. Fonction d’enrichissement + affichage
async function _fetchOppInner(){   /* ← appelé par wrapper ci-dessus */
  ...
  for(let i=0; i<candidates.length && enriched.length<50; i++){
    ...
    try{
      ...
      const p       = dataMacd?.Data?.Data?.[0] || {};
      const macd    = p.MACD || 0, signal = p.Signal || 0;
      ...
      if(forecast >= 0){
        enriched.push({ name:sym, forecast:forecast.toFixed(1), confidence, headline:hl, dateStr:dStr, url:art.url||'' });
      }
      if(enriched.length >= 50) break;   // ← arrêt propre dès qu’on a 50
    }catch(err){
...
// 4. On remplace la fonction originale appelée au chargement
...
async function refreshAll(){
  ...
  await fetchOpportunities();    // charge/rafraîchit les opportunités
}
