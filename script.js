...
/* == 1. CONFIG GÉNÉRALE ========================================= */
const API_BASE = 'https://dashboard-ia-backend.onrender.com';   // <-- votre back Render
const WS_URL   = API_BASE.replace(/^http/, 'ws') + '/ws/prices';

const PROXY           = 'https://proxi-api-crypto.onrender.com/proxy/'; // ← ancien proxy
...
const live = {};   // { BTC:{p:64500, ts:Date.now(), src:'binance'} }

(() => {
  const ws = new WebSocket(WS_URL);
  ws.onopen    = () => debug('🔌 WebSocket connecté');
  ws.onerror   = e  => debug('⚠️ WebSocket error');
...
async function fetchCrypto(sym, curr){
  ...
  const r = await safeFetch(`${API_BASE}/api/crypto/binance/${sym}`,'Binance');
...
/* == 5. PRÉ-SÉLECTION TICKERS (CoinPaprika / Gecko) ============== */
...
/* 6-0  Verrou anti-collision */
let isFetchingOpportunities = false;

/* 6-1  Wrapper anti exécution parallèle */
...
/* 6-2  Fallback CryptoPanic (hot → trending) */
...
async function _fetchOppInner(){
  ...
  const candidates = filtered.map(t=>({
    ...
    .sort((a,b)=>b.preScore-a.preScore).slice(0,100);
  ...
  for(let i=0;i<candidates.length && enriched.length<50;i++){
    ...
    try{
      ...
      if(forecast>=0){
        enriched.push({name:sym,forecast:forecast.toFixed(1),confidence:conf,headline:hl,dateStr:dStr,url:art.url||''});
      }
...
/* 6-4  Remplace l’ancienne fonction */
...
async function refreshAll(){
  ...
  for(const a of portfolio){
    ...
    const v=info.price*a.qty, gain=v-a.inv,
          cls=gain>=0?'gain':'perte', sign=gain>=0?'+':'',
          liveTag = info.live?'⚡':'';
    ...
    (a.type==='crypto'?tC:tA).innerHTML += rowHTML;
